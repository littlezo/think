...
class Poster
{
	...
	{
		return imagecolorallocatealpha($this->poster, $red, $green, $blue, $alpha);
	}
	...
	public function imageCopy($image_path, $x, $y, $width, $height, int $radius = 0)
	{
		...
		if ($radius > 0) {
			$image = $this->radius($image_path, $radius);
		}
	...
	public function imageText(string $text, int $size, array $color, int $x, int $y, $max_width = 0, $max_line = 1, $blod = false)
	{
		...
		if ($blod) {
			imagettftext($this->poster, $size, 0, ($x + 1), ($y + 1), $color, PUBLIC_PATH . 'static/font/Microsoft.ttf', $text);
		}
	}
	...
	{
		return imagecolorallocate($this->poster, $red, $green, $blue);
	}
